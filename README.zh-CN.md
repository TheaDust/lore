...
`git pull`（或重新 clone）是常规升级路径；你的 `.lore/` 在升级中保持原样。如果某个发布包含破坏性变更，发布说明（与 `CHANGELOG.md`）会说明改了什么、需要手动改什么。当前 schema 是 `schema_version: 1`；还没有任何迁移工具发布，所以今天 pull 之后你不需要跑任何东西。完整版本策略见 [`references/compatibility.md`](references/compatibility.md)。
