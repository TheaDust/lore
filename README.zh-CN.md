...
mem-man 维护一个单一事实源（`.mem-man/`），并把它投影到你的 agent 已经读取的配置文件里。它追踪**为什么**做某个决策，而不只是代码**做了什么**，并把这个历史跨 session、跨工具保留下来。
...
# 1. 初始化（每个项目运行一次）
mem-man init
# 扫描项目，生成 entry 草案，请用户确认，创建 .mem-man/
...
# 4. 强制刷新 mirror（比如手动编辑了 .mem-man/ 之后）
mem-man mirror
...
```bash
mem-man query    # 从记忆库回答问题，引用 entry ID
mem-man audit    # 检查记忆与现实的偏差，报告写入 .mem-man/audit/
```
...
| 命令 | 作用 | 写什么 |
|---|---|---|
| `init` | 首次扫描项目；生成 entry 草案；用户确认 | `.mem-man/*` + 平台 mirror |
| `sync` | 检测代码变更；提议更新；用户裁决 | 只写 `.mem-man/*`（不写 mirror）|
...
要恢复老行为（每次 sync 都更新 mirror），在 `.mem-man/.config.json` 里设 `"sync_updates_mirror": true`。
...
mem-man 的事实源是 `.mem-man/*`，但它会投影到 agent 已经读取的配置文件：
...
| LangGraph / DeepAgents |（无文件 — 直接读 `.mem-man/*.md`）| n/a |

每个 mirror 文件用 `---` 分隔符切成两段：

```markdown
## Mem-man (auto-managed)
... Skill 从 .mem-man/ 写入的内容 ...
...
Skill 只写 `## Mem-man` 段。`## My notes` 段以下都是你自由编辑的区域，Skill 在每次 sync 和 compress 时原样保留。
...
`.mem-man/.config.json` 是可选的。默认值适合大多数项目。
...
A: 那些是扁平的规则列表。mem-man 是结构化的（架构 / 决策 / 约定）、原子的（一条事实一个 entry）、有历史的（每条 entry 有 `#added` 和 `#verified` tag）。而且 mem-man 会替你生成这些文件。

**Q: mem-man 会调用 agent 的 API 吗？**
A: 不会。mem-man 是纯文件 I/O。调用 mem-man 的 agent 做语义工作（扫描代码、决定提取什么、分类变更）；mem-man 提供文件布局、ID 方案、标记规则和验证脚本。

**Q: agent 原生的 `/init` 或 `/compact` 呢？**
A: 它们用途不同。`/init` 是一次性项目扫描 → `CLAUDE.md`。`/compact` 压缩对话上下文。mem-man 的 `init` 和 `compress` 管长期项目知识，不是会话上下文。如果你在已经有非 mem-man `CLAUDE.md` 的项目上跑 `mem-man init`，接管检测（init step 0）会处理集成。
