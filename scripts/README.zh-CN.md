...
跨平台 Python 3.6+ 辅助脚本，减少重复的机械工作。无第三方依赖。被 `init` / `sync` / `audit` / `compress` / `mem-man mirror` 调用，也可独立运行做临时检查。
...
**默认只读。** 这些脚本不写 `.mem-man/`，只观察。Agent 决定如何处理发现的问题。

**从项目根目录运行。** `list_entries.py` 向上遍历定位 `.mem-man/`。其他脚本通过 subprocess 调用它，所以这个约束会传递生效。
...
| `find_stale.py` | audit 步骤 2；compress 步骤 2；mem-man mirror（可选）| 找出过期 entry 或已标记 `#stale` 的 entry |

## 测试

没有真实 `.mem-man/` 时，可以快速验证 import 和参数解析是否正常：
...
`list_entries.py`、`find_duplicates.py`、`find_stale.py` 需要有内容的 `.mem-man/` 才能产出有意义的输出。先用 `mem-man init` 建一个。
...
- **不自动 archive。** 脚本会报告待 archive 的 entry，但不会移动它们。实际搬迁到 `.mem-man/archive/` 仍需通过 `mem-man sync` 完成。
